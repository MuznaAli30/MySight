...
export default function FacialRecognition() {
  ...
  const [cameraStarted, setCameraStarted] = useState(false); // Camera toggle
  const [captured, setCaptured] = useState(false); // To track if a photo is captured
  ...
  const capturePhoto = useCallback(() => {
    ...
    if (imageSrc) {
      ...
      setCameraStarted(false); // Turn off the camera after capture
    } else {
  ...
  const refreshCapture = () => {
    ...
    setCameraStarted(false); // Turn off the camera and show the open camera option again
  };
